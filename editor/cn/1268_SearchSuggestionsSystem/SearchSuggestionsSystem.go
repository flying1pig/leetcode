...
输出：[[],[],[],[],[],[],[]]
 ...
 Related Topics 字典树 数组 字符串 二分查找 排序 堆（优先队列） 👍 205 👎 0
...
func (node *prefixTree) insert(word string) {
	for _, i := range word {
		index := int(i) - int('a')
		if node.next[index] == nil {
			node.next[index] = &prefixTree{}
		}
...
func (node *prefixTree) search(word string) [][]string {
	var res [][]string
	flag := false
	for _, ch := range word {
		if flag {
